...
namespace kalshi
{
  ...
  namespace
  {
                                                       ...
                                                       std::string_view message)
    {
      ...
      if (EVP_DigestSignInit(ctx.get(), &pkey_ctx, EVP_sha256(), nullptr, pkey) <=
          0)
      {
        set_last_sign_error_from_openssl();
        return std::unexpected(AuthError::SigningFailed);
    ...
    std::expected<std::string, AuthError> base64_encode(const std::string &bytes)
    {
      ...
      int out_len =
          EVP_EncodeBlock(reinterpret_cast<unsigned char *>(b64.data()),
  ...
  {
    return g_last_sign_error.empty() ? "no error detail"
                                     : g_last_sign_error.c_str();
  }
  ...
  std::expected<std::string, AuthError>
  sign_ws_message(std::string_view private_key_pem, std::string_view message)
  {
    ensure_legacy_provider_loaded();
    if (is_openssh_key(private_key_pem))
    {
      g_last_sign_error = "OpenSSH private key format detected; convert to PEM "
                          "(PKCS#8) for OpenSSL";
      return std::unexpected(AuthError::SigningFailed);
    }
  ...
  std::expected<std::vector<Header>, AuthError>
  build_ws_headers(const AuthConfig &auth, std::string_view path,
