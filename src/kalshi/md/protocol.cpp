...
{
  return std::find(channels.begin(), channels.end(), ORDERBOOK_DELTA) != channels.end();
}

} // namespace

SubscriptionCommand::SubscriptionCommand(SubscribeRequest request)
  : request_(std::move(request)), json_(build_json(request_))
{
}

std::expected<SubscriptionCommand, SubscribeError> SubscriptionCommand::from_config(
    const kalshi::Config& config, int id)
{
  auto request = build_request(config, id);
  if (!request)
  {
...
std::expected<SubscribeRequest, SubscribeError> SubscriptionCommand::build_request(
    const kalshi::Config& config, int id)
{
...
std::string SubscriptionCommand::build_json(const SubscribeRequest& req)
{
